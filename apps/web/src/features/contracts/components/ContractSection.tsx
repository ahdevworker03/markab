...
export function ContractSection({ rentalId }: ContractSectionProps) {
  ...
  async function handleDownloadDocument(doc: DocumentResponse) {
    ...
    try {
      ...
      triggerDownload(
        blob,
        doc.originalFilename || `signed-contract-${doc.id}`,
      );
    } catch (err) {
      setLocalError(getApiErrorMessage(err).title);
    } finally {
  ...
  return (
    <SectionCard
      ...
      action={
        data ? (
          <span className="number-ltr text-xs text-muted-foreground">
            #{data.id.slice(0, 8)}
          </span>
        ) : undefined
      }
    >
      <div className="space-y-4">
        ...
        {contract.query.isLoading ? (
          ...
          !(
            contract.query.error instanceof ApiError &&
            contract.query.error.status === 404
          ) ? (
          ...
          <div className="text-center py-6 space-y-3">
            <FileText
              className="w-10 h-10 text-muted-foreground mx-auto"
              strokeWidth={1.5}
            />
            <p className="text-sm text-muted-foreground">
              لا يوجد عقد لهذا الإيجار
            </p>
            {isOwner && (
              <button
          ...
          <>
            ...
            <div className="space-y-1">
              ...
              <InfoRow
                ...
                value={formatDateTime(data.pickupDate)}
              />
              <InfoRow
                label="تاريخ الإرجاع"
                value={formatDateTime(data.expectedReturnDate)}
              />
            </div>
            ...
            <div className="border-t border-border pt-3">
              ...
              {signed.query.isLoading ? (
                ...
                !(
                  signed.query.error instanceof ApiError &&
                  signed.query.error.status === 404
                ) ? (
                      ...
                      <div className="bg-muted rounded-xl p-3 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground mt-0.5">
                            <span dir="ltr" className="number-ltr">
